...
console.log(JSON.stringify(itemLst)+'itemLst');
...
console.log(JSON.stringify(this.itemListmap)+'map');
      ...
      //const d = this.productData.filter(element => !dataindexToRemove.includes(element.recordIndex));
        ...
        console.log(JSON.stringify(this.itemListmap)+'beforethis.itemListmap');
        console.log(JSON.stringify(this.itemListmapbackup)+'this.itemListmapbackup');
        ...
        console.log(JSON.stringify(this.productData)+'productdta');
        for (var i = 0; i < this.productData.length; i++) {
          var obj = { ...this.productData[i] };
        ...
        if (event.detail.name == "Discounts") {
          ...
          if(this.itemListmap[this.inlinerecordindex] && obj.selectedDropdownValue=='Volume' ){
            ...
            console.log(JSON.stringify(this.itemListmap)+'this.itemListmap');
            console.log(this.inlinerecordindex+'inlinerecordindex');
            console.log(JSON.stringify(this.itemList)+'itemlist');
            this.Volumediscount=true;
          }
        ...
        const productDataWithIndex = this.selectedrows.map((record, i) => {
          ...
          let itemlstrec=this.itemListmap[record.recordIndex];
           ...
           let index= this.index++;
            let aa= itemlstrec.filter((record) =>{
        ...
        this.selectedids = selectedIds;
         console.log('2'+JSON.stringify(this.selectedids));
        ...
        console.log('6');
          for(let selectedid of  this.selectedids){
            ...
            let id=selectedid.toString();
             console.log('6.2'+id);
             console.log(JSON.stringify(this.itemListmap)+'6');
             console.log(this.itemListmap.has(id)+'6');
             this.itemListmap.delete(id);
             console.log('6.3'+JSON.stringify(this.itemListmap));
            console.log(JSON.stringify(this.itemListmap)+'id');
          }
          console.log('7');
           console.log(JSON.stringify(this.itemListmap)+'1797');
        let datawithindex = { productdata: arr, index: this.index };
          console.log(JSON.stringify(this.selectedids)+'this.selectedids');
        console.log(JSON.stringify(this.itemListmap)+'this.itemlistmap');
        console.log('8');
...
console.log(JSON.stringify(this.itemListmap) + ' this.itemListmap after update11');
      
});
console.log('9');

let k = Object.keys(this.itemListmap);  // Get keys of the object as an array
console.log(JSON.stringify(k) + ' this..keySet()');
console.log(JSON.stringify(this.itemListmap) + ' this.itemListmap after 01');

let newmap = new Map();
 console.log(JSON.stringify([...newmap]) + ' newmapbefore');
for (let key of k) {
    let filteredRecords = this.itemListmap[key].filter(record => !selectedIds.includes(record.dataindex));
newmap[key]=filteredRecords;
   // newmap.set(key, filteredRecords);
    console.log(JSON.stringify([...newmap]) + ' newmap');
}
console.log('10');
this.itemListmap = newmap;
console.log(JSON.stringify(this.itemListmap) + ' this.itemListmap after update');


        console.log(JSON.stringify(this.itemListmap)+'this.itemlistmapafter02');
